...
export default function HandoverPage() {
  ...
  // 1. Si cambia el TIPO (Entrega/Recogida), reseteamos activos y el motivo
  // (porque las opciones de motivo son distintas para cada uno)
  useEffect(() => {
    setForm((f) => ({ ...f, assetIds: [], reason: '' }));
  }, [form.type]);

  // 2. Si cambia la PERSONA, SOLO reseteamos los equipos seleccionados
  // (porque el inventario a mostrar cambia), pero respetamos el motivo.
  useEffect(() => {
    setForm((f) => ({ ...f, assetIds: [] }));
  }, [form.personId]);
  ...
  const create = useMutation({
    ...
    onError: (e: any) => {
      ...
      // Añadimos e?.message para que atrape validaciones del cliente
      toast.error(server?.error || server?.details?.message || e?.message || 'No se pudo registrar.');
