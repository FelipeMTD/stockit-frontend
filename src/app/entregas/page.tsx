...
export default function HandoverPage() {
  ...
  const routeIdsToFetch = useMemo(() => {
    const ids: string[] = [];
    (handoversQ.data || []).forEach((h: any) => {
      const match = h.notes?.match(/Generada desde Entregas y Recogidas ([a-zA-Z0-9_-]+)/i);
    });
    return ids; 
  }, [handoversQ.data]);

  const routesQuery = useQuery({
    queryKey: ['all-routes-for-handovers'],
    queryFn: async () => {
      const res = await api.get('/api/routes', { params: { pageSize: 500 } });
      const routesDict: Record<string, any> = {};
      res.data?.items?.forEach((r: any) => {
        const match = r.notes?.match(/Generada desde Entregas y Recogidas ([a-zA-Z0-9_-]+)/i);
        if (match && match[1]) {
          routesDict[match[1].trim()] = r;
        }
      });
      return routesDict;
    },
    enabled: !!handoversQ.data?.length,
  });
  ...
  function buildPayload(f: FormState) {
    const base: any = {
      ...
      scheduledDate: f.homeDelivery && f.scheduledDate ? f.scheduledDate : null,
  ...
  const create = useMutation({
    mutationFn: async (vars: { formState: FormState; fileToUpload: File | null }) => {
      ...
      if (formState.homeDelivery && !formState.scheduledDate) throw new Error('Selecciona la fecha programada de la ruta a domicilio');
              ...
              {!form.homeDelivery && (
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input className="rounded-xl border px-3 py-3 text-sm bg-slate-50 focus:border-sky-600 outline-none uppercase font-bold" placeholder="Nombre de quien recibe" value={form.signerName} onChange={(e) => setForm({ ...form, signerName: e.target.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ\s]/g, '') })} />
                    <input inputMode="numeric" className="rounded-xl border px-3 py-3 text-sm bg-slate-50 focus:border-sky-600 outline-none" placeholder="N° de Cédula" value={form.signerId} onChange={(e) => setForm({ ...form, signerId: e.target.value.replace(/\D/g, '') })} />
                  </div>
            ...
            {handoversQ.isLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {(handoversQ.data || []).map((h: any) => {
                  const linkedRoute = routesQuery.data?.[h.id];
                  return (
                  <div key={h.id} className="border border-slate-200 rounded-2xl p-5 bg-white shadow-sm flex flex-col gap-3 transition-shadow hover:shadow-md">
                    ...
                    <div>
                      ...
                      {linkedRoute?.scheduledDate && (
                         <p className="text-[11px] font-bold text-sky-700 mt-1 bg-sky-50 inline-block px-2 py-0.5 rounded border border-sky-100">
                           Programada: {new Date(linkedRoute.scheduledDate).toLocaleDateString('es-CO')}
                         </p>
                      )}
                    </div>
              ...
              <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4 sm:space-y-6">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 sm:p-5 space-y-4">
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <a 
                      href={getSecureUrl(`/uploads/handovers/Comodato_${selectedHandover.id}.pdf`)} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className={`w-full flex items-center justify-center bg-indigo-50 border-2 border-indigo-200 text-indigo-700 hover:bg-indigo-100 transition-all font-black uppercase text-[11px] sm:text-xs tracking-widest py-3 rounded-xl shadow-sm gap-2 ${!(selectedHandover.attachmentPath && !selectedHandover.attachmentPath.includes('Comodato_')) ? 'sm:col-span-2' : ''}`}
                    >
                      Ver Comodato
                    </a>

                    {selectedHandover.attachmentPath && !selectedHandover.attachmentPath.includes('Comodato_') && (
                      <a 
