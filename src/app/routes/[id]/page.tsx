...
export default function RouteDetailPage() {
  ...
  useEffect(() => {
    ...
    if (isConductor && !isPendingReview && !isCompletedFinal) {
      ...
      setInitialized(true);
    } else if (isPendingReview && sigData) { 
  ...
  const handleComplete1_2 = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalNotes = [(data as any).notes || '', detailNotes].filter(Boolean).join(' || ');
      ...
      await api.patch(`/api/routes/${id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' }});
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 font-poppins text-slate-900 pb-20 pt-4 px-4">
      ...
      {/* ✅ AQUÍ ESTÁ LA LISTA DE EQUIPOS VISIBLE PARA EL CREADOR CUANDO ESTÁ PROGRAMADA */}
          ...
          <div className="grid gap-3">
            {allItems.map((it: any) => (
              <div key={it.id} className="flex justify-between items-center py-3 border-b border-slate-50 last:border-0">
                <span className="text-sm font-bold text-slate-700">{it.asset?.tag || 'S/N'} <span className="text-xs font-medium text-slate-500 uppercase ml-1">• {it.asset?.name || 'EQUIPO NO IDENTIFICADO'}</span></span>
      ...
      {/* REPORTE EN CALLE (VISIBLE PARA TODOS EN PASO 1/2) */}
      {isPendingReview && sigData && (
        <section className="bg-sky-50 border border-sky-100 rounded-xl p-6 shadow-sm space-y-4">
          <h2 className="text-sm font-bold text-sky-900 uppercase tracking-widest border-b border-sky-200 pb-2">Reporte de Gestión en Calle</h2>
          <div className="grid gap-3">
            {allItems.map((it: any) => {
              const wasReported = sigData.driverAssets.includes(it.asset.id);
              return (
                <div key={it.id} className="flex justify-between items-center py-2 border-b border-sky-200/50 last:border-0">
                  <span className={`text-sm ${wasReported ? 'font-bold text-slate-700' : 'text-slate-400 line-through'}`}>{it.asset?.tag} - {it.asset?.name}</span>
                  <span className={`text-[9px] font-black px-2 py-1 rounded ${wasReported ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>{wasReported ? 'CONFIRMADO' : 'OMITIDO'}</span>
                </div>
              )
            })}
          </div>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div className="grid gap-1.5 sm:col-span-2">
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex justify-between">
                  Foto / Evidencia Adicional (Opcional)
                  {photoPreview && <button type="button" onClick={() => { setPhotoPreview(null); setPhotoFile(null); }} className="text-rose-500 underline">Borrar Foto</button>}
                </label>
                {!photoPreview ? (
                  <input type="file" accept="image/*" capture="environment" onChange={handlePhotoChange} className="w-full rounded-xl border border-slate-300 px-3 py-3 text-sm bg-slate-50 cursor-pointer file:mr-3 file:rounded-lg file:border-0 file:bg-sky-100 file:text-sky-700 file:px-3 file:py-1.5 file:text-xs file:font-bold" />
                ) : (
                  <div className="relative w-fit">
                    <img src={photoPreview} alt="Evidencia" className="h-32 object-cover border-4 border-slate-200 rounded-xl shadow-sm" />
                  </div>
                )}
              </div>
      ...
      {isPendingReview && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          <section className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-5">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b pb-2">Receptor Físico (Datos)</h2>
            ...
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
              ...
              {sigData?.photoData && (
                <div className="mt-4 pt-4 border-t border-slate-200">
                  <p className="text-[9px] font-bold text-slate-400 uppercase mb-2">Evidencia Fotográfica:</p>
                  <a href={getSecureUrl(sigData.photoData)} target="_blank" rel="noopener noreferrer">
                    <img src={getSecureUrl(sigData.photoData)} alt="Evidencia" className="h-32 object-cover border-4 border-white shadow-md rounded-xl hover:scale-105 transition-transform cursor-pointer" />
                  </a>
                </div>
              )}
            </div>
      ...
      {isCompletedFinal && sigData && (
        <section className="bg-white rounded-2xl border border-emerald-100 shadow-2xl overflow-hidden animate-in fade-in duration-500">
          <div className="bg-emerald-600 p-6 text-white flex items-center justify-between shadow-lg"><div><h2 className="text-xl font-black uppercase tracking-tighter">Servicio Finalizado</h2><p className="text-[10px] font-bold opacity-80 uppercase tracking-widest">Comodato enviado y ruta cerrada</p></div><div className="h-12 w-12 bg-white/20 rounded-full flex items-center justify-center text-2xl font-black">COMPLETADO</div></div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                ...
                {sigData.photoData && (
                  <div className="mt-5 pt-4 border-t border-slate-100">
                    <p className="text-[9px] font-bold text-slate-400 uppercase mb-2">Evidencia Fotográfica:</p>
                    <a href={getSecureUrl(sigData.photoData)} target="_blank" rel="noopener noreferrer">
                      <img src={getSecureUrl(sigData.photoData)} alt="Evidencia" className="h-24 object-cover border-4 border-white shadow-sm rounded-xl cursor-pointer" />
                    </a>
                  </div>
                )}
              </div>
                ...
                {allItems.map((it: any) => {
                  ...
                  return (
                    <div key={it.id} className={`flex items-center justify-between p-4 rounded-xl border-2 transition-all ${wasFinalized ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-white border-slate-100 opacity-60'}`}>
                      ...
                      <span className={`flex items-center justify-center px-3 py-1 text-xs font-bold rounded-lg border shadow-sm ${wasFinalized ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
